...
const db = require('monk')('localhost:27017/quote');
...
io.sockets.on('connection', socket => {
  ...
  socket.on('PLAYERS_SHUFFLE', async ({ room }) => {
    const { players } = await rooms.findOne({ room }, { players: 1 });
  ...
  socket.on('DRAFT_START', async ({ room }) => {
    const { players } = await rooms.findOne({ room }, 'players');
  ...
  socket.on('PLAYER_ADD_CHARACTER', async ({ room, payload }) => {
    ...
    const roomState = await rooms.findOneAndUpdate({ room }, roomData);
